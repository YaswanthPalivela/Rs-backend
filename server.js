...
app.get("/getTableTransactions", async (req, res) => {
  try {
    ...
    res.status(400);
    res.send({ error: `something went to wrong please try again` });
    console.log(e);
  }
});
...
app.get("/getStatistics", async (req, res) => {
  try {
    ...
    const responseOfCountOfNoneOfDate = await db.get(queryOfCountOfNoneSold);
    res.status(200);
    rep.send({
    ...
    res.status(400);
    res.send({ error: `someting went to wrong please try again` });
  }
});
